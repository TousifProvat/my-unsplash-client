export const uploadConstants = {
  GET_IMAGES_REQUEST: 'GET_IMAGES_REQUEST',
  GET_IMAGES_SUCCESS: 'GET_IMAGES_SUCCESS',
  GET_IMAGES_FAILURE: 'GET_IMAGES_FAILURE',
  GET_IMAGE_BY_LABEL_REQUEST: 'GET_IMAGE_BY_LABEL_REQUEST',
  GET_IMAGE_BY_LABEL_SUCCESS: 'GET_IMAGE_BY_LABEL_SUCCESS',
  GET_IMAGE_BY_LABEL_FAILURE: 'GET_IMAGE_BY_LABEL_FAILURE',
  IMAGE_UPLOAD_REQUEST: 'IMAGE_UPLOAD_REQUEST',
  IMAGE_UPLOAD_SUCCESS: 'IMAGE_UPLOAD_SUCCESS',
  IMAGE_UPLOAD_FAILURE: 'IMAGE_UPLOAD_FAILURE',
  DELETE_IMAGE_REQUEST: 'DELETE_IMAGE_REQUEST',
  DELETE_IMAGE_SUCCESS: 'DELETE_IMAGE_SUCCESS',
  DELETE_IMAGE_FAILURE: 'DELETE_IMAGE_FAILURE',
};
